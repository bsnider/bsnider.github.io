  ...
  require(["esri/Map",
    ...
    SlideList, ContentPane) {
      ...
      function addIconLayer() {
        ...
        app.sceneView.whenLayerView(fl).then(function (lyrView) {
          lyrView.watch("updating", function (val) {
            if (!val && !loaded) { // wait for the layer view to finish updating

              app.sceneView.on("click", function (response) {
                var mapPoint = {
                ...
                var query = new Query();
                query.geometry = pointToExtent(app.sceneView, mapPoint, 40);
                lyrView.queryFeatures(query)
                  .then(function (res) {
                    ...
                    if (geomArray.length == 1) {
                      ...
                      app.sceneView.goTo(geomArray);
                      //   app.sceneView.goTo(geomArray).then(function() {
                      ...
                      app.sceneView.popup.watch('featureCount', function (newValue, oldValue, property, object) {
                        //debugger;
              ...
              var slideList = new SlideList(options, "carousel");
              slideList.startup();
              loaded = true;
            }
          });
      ...
      // Search
      app.s = createSearchWidget("searchNavDiv");
      ...
      function setPanelVisibility() {
        var isMobileScreen = app.sceneView.widthBreakpoint === "xsmall" || app.sceneView.widthBreakpoint === "small",
          ...
          isDockedBottom = app.sceneView.popup.currentDockPosition && app.sceneView.popup.currentDockPosition.indexOf("bottom") > -1;
        // Mobile (xsmall/small)
        if (isMobileScreen) {
      ...
      query(".calcite-panels .panel").on("show.bs.collapse", function (e) {
        if (app.sceneView.popup.currentDockPosition || app.sceneView.widthBreakpoint === "xsmall") {
          app.sceneView.popup.dockEnabled = false;
        }
      });

      // Panels - Undock popup when panels hide (mobile only)
      query(".calcite-panels .panel").on("hide.bs.collapse", function (e) {
        if (app.sceneView.widthBreakpoint === "xsmall") {
          app.sceneView.popup.dockEnabled = true;
        }
      });
      ...
      query(".esri-popup .esri-title").on("click", function (e) {
        ...
        app.sceneView.popup.reposition();
      });
